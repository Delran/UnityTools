...
public class CM_CameraManager : MonoBehaviour
{
    ...
    public void EnableCamera(string _id)
    {
        if (cameras.ContainsKey(_id))
            cameras[_id].IsEnabled = true;
    }

    public void DisableCamera(string _id)
    {
        if (cameras.ContainsKey(_id))
            cameras[_id].IsEnabled = false;
    }

    public void SwitchCamera(string _id)
    {
        if (!cameras.ContainsKey(_id)) return;
        foreach (KeyValuePair<string, CM_CameraComponent> cam in cameras)
        {
            cam.Value.SwitchView(false);
        }
        cameras[_id].SwitchView(true);
    }
